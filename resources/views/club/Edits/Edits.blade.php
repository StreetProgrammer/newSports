...
@section('content')
  ...
  <div id="pageTop">
    @if (!Auth::check())
      ...
      @include('club.Edits.pageParts.mainRegisterAdvice')

    @else
    <!--  -->
      @include('club.Edits.pageParts.rejectedMessage')
    <!--  -->
    @endif
  </div>
  <div class="">
    ...
    <div id="contentChangable">
      @if (Auth::check())
        ...
        @php
          $club = Auth::user();
        @endphp
        @include('club.Edits.pageParts.editMainRegisterInfo')

      @else

        @include('club.Edits.pageParts.mainRegisterInfo')
...
@section('page_specific_scripts')
    <!-- flot charts scripts-->
    <script src="{{ url('/') }}/club/js/clubEdit.js"></script>
@stop